...
private[spark] object BLAS extends Serializable with Logging {
  ...
  private def gemm(
    ...
    val containerID = sys.env("CONTAINER_ID");
    val jvm_PID = sys.env("JVM_PID");
    ...
    println(s"[OpenBlasgemm] TID:$tid:mode:openblas:compute:$diff:getresult:unknown:modeName:native:JVMID:$jvm_PID:containerID:$containerID")
    ret
  }
